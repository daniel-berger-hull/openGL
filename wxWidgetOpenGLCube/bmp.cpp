...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	wchar_t* convertedFilename = charToWChar(Filename);	
	HANDLE DIBHandle = LoadImage(0, convertedFilename, IMAGE_BITMAP, 0, 0,LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);  // use windows to get header info of bitmap - assume no errors in header format 
