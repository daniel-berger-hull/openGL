...
void OpenGLCanvas::OnPaint(wxPaintEvent& WXUNUSED(event))
{
	...
	Point3D  lightLoc = light->getLocation();

	light0_pos[0] = lightLoc.x;
	light0_pos[1] = lightLoc.y;
	light0_pos[2] = lightLoc.z;
	light0_pos[3] = 0.0f;
...
void OpenGLCanvas::OnMouse(wxMouseEvent& event)
{
	if (event.Dragging())
	{
		...
		ControlPanel* control =   GeneralModel::getInstance()->getControlPanel();
		
		GeneralModel::getInstance()->refreshControlPanel();
...
void OpenGLCanvas::InitGL()
{
	...
	//glEnable(GL_COLOR_MATERIAL);  // Put back if you want the color
